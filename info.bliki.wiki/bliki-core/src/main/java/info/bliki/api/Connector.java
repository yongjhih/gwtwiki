...
public class Connector {
  ...
  public User login(User user) {
    ...
    if (lgDomain.length() > 0) {
      params = new NameValuePair[] { new NameValuePair("action", "login"),
          new NameValuePair("format", "xml"),
          new NameValuePair("lgname", userName),
          new NameValuePair("lgpassword", user.getPassword()),
  ...
  public List<Page> queryContent(User user, List<String> listOfTitleStrings) {
    String[] valuePairs = { "prop", "revisions", "rvprop",
  ...
  public List<Page> queryImageinfo(User user, List<String> listOfImageStrings,
      int imageWidth) {
    String[] valuePairs = { "prop", "imageinfo", "iiprop", "url", "iiurlwidth",
  ...
  private List<Page> queryInfoWithEditToken(User user, String title) {
    Query query = Query.create().prop("info", "revisions").titles(title)
        .intoken("edit");
   ...
   * @return
   */
  public List<Page> query(User user, List<String> listOfTitleStrings,
      String[] valuePairs) {
    try {
   ...
   *         otherwise
   */
  public String queryXML(User user, List<String> listOfTitleStrings,
      String[] valuePairs) {
    ...
    if (titlesString.length() > 0) {
      ...
      method.addParameter(new NameValuePair(PARAM_TITLES, titlesString
          .toString()));
    }
    if (valuePairs != null && valuePairs.length > 0) {
      for (int i = 0; i < valuePairs.length; i += 2) {
        method
            .addParameter(new NameValuePair(valuePairs[i], valuePairs[i + 1]));
      }
    }
          ...
          try {
            if (response != null) {
              ...
              if (errorData != null) {
                ...
                UnexpectedAnswerException ex = new UnexpectedAnswerException(
                    errorData.getInfo());
  ...
  private PostMethod createAuthenticatedPostMethod(User user) {
    ...
    method.addParameter(new NameValuePair(PARAM_LOGIN_USERNAME, user
        .getUserid()));
    method.addParameter(new NameValuePair(PARAM_LOGIN_USERID, user

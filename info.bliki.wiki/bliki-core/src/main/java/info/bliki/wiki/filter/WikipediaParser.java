...
public class WikipediaParser extends AbstractParser implements IParser {
   ...
   * 
   */
  private void createContentToken(boolean whiteStart,
      final int whiteStartPosition, final int diff) {
    if (whiteStart) {
      try {
        ...
        if (count > 0) {
          fWikiModel.append(new ContentToken(fStringSource.substring(
              whiteStartPosition, whiteStartPosition + count)));
        }
      } finally {
        fWhiteStart = false;
      }
      ...
      while (true) {
        ...
        if (isStartOfLine() && fWikiModel.getRecursionLevel() == 1) {
          if (fWikiModel.stackSize() > 0
              && (fWikiModel.peekNode() instanceof PTag) && isEmptyLine(1)) {
  ...
  private boolean parseHTMLCommentTags() {
    int htmlStartPosition = fCurrentPosition;
    String htmlCommentString = fStringSource.substring(fCurrentPosition - 1,
        fCurrentPosition + 3);

    if (htmlCommentString.equals("<!--")) {
      fCurrentPosition += 3;
      if (readUntil("-->")) {
        String htmlCommentContent = fStringSource.substring(
            htmlStartPosition + 3, fCurrentPosition - 3);
        if (htmlCommentContent != null) {
          createContentToken(fWhiteStart, fWhiteStartPosition, fCurrentPosition
              - htmlStartPosition + 1);
          return true;
        }
      }
  ...
  private boolean parseURIScheme() {
    ...
    if (foundUrl) {
      ...
      if (fWikiModel.isValidUriSchemeSpecificPart(uriSchemeName,
        ...
        fCurrentPosition = tempPosition;
        fCurrentPosition--;
        fWikiModel.appendExternalLink(uriSchemeName, restString, restString,
            true);
        return true;
      }
  ...
  private boolean parseCamelCaseLink() {
    ...
    if (isCamelCase) {
      ...
      String name = fStringSource
          .substring(startLinkPosition, fCurrentPosition);
  ...
  private boolean parseWikiTag() {
    ...
    if (findWikiLinkEnd()) {
      ...
      fEventListener
          .onWikiLink(fSource, startLinkPosition, endLinkPosition, "");
  ...
  private boolean parseDefinitionLists() {
    if (isStartOfLine()) {
      ...
      if (readUntilEOL()) {
        ...
        reduceTokenStack(dl);
        String head = fStringSource.substring(startHeadPosition,
            fCurrentPosition);
        ...
        if (index > 0) {
          ...
          WikipediaParser.parseRecursive(head.substring(0, index).trim(),
              fWikiModel, false, true);
          // fResultBuffer.append("&nbsp;</dt><dd>");
          fWikiModel.popNode();
          fWikiModel.pushNode(new DdTag());
          WikipediaParser.parseRecursive(head.substring(index + 2).trim(),
              fWikiModel, false, true);
          ...
          if (index > 0) {
            ...
            fWikiModel.pushNode(new DdTag());
            // fResultBuffer.append("</dt><dd>");
            WikipediaParser.parseRecursive(head.substring(index + 1).trim(),
                fWikiModel, false, true);
            fWikiModel.popNode();
            fWikiModel.popNode();
            // fResultBuffer.append("\n</dd></dl>");
            ...
            // fResultBuffer.append("<dl><dt>");
            WikipediaParser
                .parseRecursive(head.trim(), fWikiModel, false, true);
            fWikiModel.popNode();
            fWikiModel.popNode();
    ...
    if (fSource[fCurrentPosition] == '{') {
      ...
      if (fSource[templateStartPosition] != '{') {
        ...
        if (templateEndPosition > 0) {
          fEventListener.onTemplate(fSource, templateStartPosition,
    ...
    if (name != null) {
      ...
      if (foundUrl) {
        ...
        if (isEmail) {
          ...
          if (fWikiModel.isValidUriSchemeSpecificPart(uriSchemeName,
              uriSchemeSpecificPart)) {
            fWikiModel.appendExternalLink(uriSchemeName, urlString, alias,
                false);
            return true;
          }
        }
      ...
      if (tag instanceof EndTagToken) {
        ...
        if (null != bodyString) {
          if (tag instanceof INoBodyParsingTag) {
            ...
            WikipediaParser.parseRecursive(bodyString.trim(), fWikiModel,
                false, true);
          }
        }
  ...
  private void reduceTokenStackBoldItalic() {
    ...
    while (fWikiModel.stackSize() > 0) {
      ...
      if (token.equals(BOLD) || token.equals(ITALIC)
  ...
  public static void parse(String rawWikiText, IWikiModel wikiModel,
    ...
    try {
      ...
      if (templateParserBuffer != null) {
        ...
        buf = new StringBuilder(rawWikiText.length() + rawWikiText.length()
            / 10);
      }
      if (parseTemplates) {
        ...
        try {
          ...
          ioe.printStackTrace();
          pass1Text = "<span class=\"error\">TemplateParser exception: "
